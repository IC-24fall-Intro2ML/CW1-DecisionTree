...
class DecisionTreeClassifier:
    ...
    def evaluate_metrics(self, tree, X_test_y):
        ...
        accuracy, recalls, precisions, f1_scores = calculate_metrics_from_confusion_matrix(
            confusion_matrix)
    ...
    def visualize_tree(self, node=None, depth=0, x=0.5, y=1.0, x_offset=0.1, ax=None, train_data=None):
        ...
        if node.feature is None:
            ax.text(x, y, f'Label {node.label}', bbox=dict(
                facecolor='white', edgecolor='black', linewidth=8), ha='center', fontsize=100)
        else:
        ...
        if depth <= 6:
            ...
            next_x_offset = x_offset / (depth/3 + 0.3)
        else:
