...
class DecisionTreeClassifier:
    ...
    def visualize_tree(self, node=None, depth=0, x=0.5, y=1.0, x_offset=0.1, ax=None):
        ...
        if node.feature is None:
            ax.text(x, y, f'Label {node.label}', bbox=dict(facecolor='white', edgecolor='black', linewidth=8), ha='center', fontsize=100)
        else:
            ax.text(x, y, f'X{node.feature}<= \n{node.value}', bbox=dict(facecolor='white', edgecolor='black', linewidth=8), ha='center', fontsize=100)
        
        next_y = y - 0.1
        if depth <= 6:
            next_x_offset = x_offset / (depth/3 + 0.3)  # Reduce x_offset as depth increases
        else:
            next_x_offset = x_offset
            
        if node.left:
            next_x_left = x - next_x_offset
            ax.plot([x, next_x_left], [y, next_y], 'k-', linewidth=15)
            self.visualize_tree(node.left, depth + 1,
                                next_x_left, next_y, x_offset, ax)
        if node.right:
            next_x_right = x + next_x_offset
            ax.plot([x, next_x_right], [y, next_y], 'k-', linewidth=15)
